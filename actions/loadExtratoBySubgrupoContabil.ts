...
function loadExtratoBySubgrupoContabil() {
  return action('loadExtratoBySubgrupoContabil', 'SQL', {
    databaseName: 'provision',
    query: `
      WITH saidas AS (
        SELECT
          COALESCE(gc.descricao, 'Sem Grupo') as grupo_nome,
          COALESCE(sc.descricao, 'Sem Subgrupo') as subgrupo_nome,
        ...
        GROUP BY gc.id, gc.descricao, sc.id, sc.descricao
      ),
      entradas AS (
        SELECT
          COALESCE(gc.descricao, 'Sem Grupo') as grupo_nome,
      ...
      ORDER BY grupo_nome, subgrupo_nome, direcao;
    `,
  });
}

export default loadExtratoBySubgrupoContabil;
