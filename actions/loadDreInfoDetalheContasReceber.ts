...
import { andIdIn, andIdInWhenPagamento, andProjetoIdInReceber, andProjetoStatusReceber } from '@/lib/sql-filters';
...
function loadDreInfoDetalheContasReceber() {
  return action('loadDreInfoDetalheContasReceber', 'SQL', {
    ...
    query: `
      ...
      LEFT JOIN (
        SELECT crp2.conta_receber_id, crp2.projeto_id FROM contas_receber_projetos crp2
        UNION
        SELECT crf2.conta_receber_id, crf2.projeto_id FROM contas_receber_faturamento crf2
      ) vinc ON vinc.conta_receber_id = cr.id
      LEFT JOIN projetos proj ON proj.id = vinc.projeto_id
      WHERE
        1=1
