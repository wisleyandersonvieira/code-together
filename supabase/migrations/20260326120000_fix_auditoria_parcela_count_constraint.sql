-- Fix: allow increasing the number of parcelas even when paid parcelas exist.
-- Previously the RPC blocked any change in count (using <>) which prevented
-- legitimate splits of partially-paid installments. Changed to > so that
-- adding new PENDENTE parcelas is allowed, while removing parcelas is still blocked.
...
BEGIN
  ...
  LOOP
    ...
    IF COALESCE((v_item->>'id')::BIGINT, 0) > 0 THEN
      ...
      -- Allow increasing parcela count (splits) but not reducing when paid parcelas exist
