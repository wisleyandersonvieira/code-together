-- Permite extornar parcelas pagas (reverter PAGO → PENDENTE)
-- A versão anterior ignorava o campo status quando a parcela já estava PAGO no banco,
-- impedindo que o extorno funcionasse mesmo com o frontend enviando status: 'PENDENTE'.
    ...
    LOOP
      ...
      IF FOUND THEN
        IF v_existing_parcela.status = 'PAGO' THEN
          IF COALESCE(v_parcela->>'status', 'PAGO') = 'PENDENTE' THEN
            -- Extornar: reverter parcela paga para pendente
            UPDATE auditoria_fornecedor_item_parcelas
            SET
              status = 'PENDENTE',
              data_pagamento = NULL,
              data_registro_baixa = NULL,
              usuario_id = NULL
            WHERE id = v_existing_parcela.id;
          ELSIF ROUND(v_existing_parcela.valor_parcela, 2) <> ROUND(COALESCE((v_parcela->>'valor_parcela')::NUMERIC, 0), 2) THEN
