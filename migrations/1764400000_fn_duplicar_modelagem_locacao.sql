-- Modelagem Financeira — duplicação com locação e múltiplas facilidades.
--
-- Substitui a função da migration 1763700000. Três mudanças, e a do meio é a que
-- produziria dado errado em silêncio:
--
--   1. `tipo_modelagem` acompanha a cópia;
--   2. `modelagem_financiamento` deixou de ser 1:1, então vira laço com mapa — e
--      `refinancia_facilidade_id` é AUTO-REFERENCIAL, exatamente como
--      `modelagem_custos.grupo_pai`: copiada direto, a facilidade da cópia
--      apontaria para a facilidade da ORIGINAL, sem erro nenhum;
--   3. as três tabelas da locação entram na cópia.
--
-- A consulta que prova a correção do item 2, e que tem de voltar VAZIA:
--
--   SELECT f.id, f.refinancia_facilidade_id FROM modelagem_financiamento f
--    WHERE f.modelagem_id = <cópia>
--      AND f.refinancia_facilidade_id IN
--          (SELECT id FROM modelagem_financiamento WHERE modelagem_id = <origem>);
--
-- Idempotente: CREATE OR REPLACE. Chamar duas vezes cria duas cópias, que é o
-- comportamento correto de "duplicar".
--
-- O restante do corpo — mapas em tabela temporária, por que não RETURNING
-- posicional, a armadilha do grupo_pai — está documentado na 1763700000 e
-- reproduzido abaixo sem alteração.
